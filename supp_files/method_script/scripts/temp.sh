...
t=3
nb_aa=26
DIR="/media/bories/Backup/bories/Documents/Travail/results/homoPOPC-aa/homoPOPC-SCT"
...
equil=200	# equilibration section
...
contacts="1"	# contacts of aa with membrane
...
if [ "$densProf" != "0" ]; then
  ...
  sed "s/AAA/$solute1/" $SCRIPTDIR/densityProfiles-aa.vmd > solutes.vmd
  
  for i in 1 2 3 4 5
  do
    ln -s bloc$i.dcd trajectory.dcd
    vmd -dispdev text -e $SCRIPTDIR/densityProfiles-popc.vmd
    vmd -dispdev text -e $SCRIPTDIR/solutes.vmd
    rm trajectory.dcd
    mv profile-aa.dat  $DATA/densityProfiles/profile-aa$i.dat
...
if [ "$orders" != "0" ]; then
  ...
  do
    ...
    ln -s bloc$i-.dcd trajectory.dcd
    vmd -dispdev text -e $SCRIPTDIR/orders-popc.vmd
    rm trajectory.dcd
