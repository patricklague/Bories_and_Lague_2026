...
for filename in file_list:
    df = pd.read_csv(filename, sep='\s+', header=None, names=["x", "y", "z"], usecols=[0,1])
    # Créer l’index complet de -50 à 50
    full_x = pd.Index(np.arange(-50.5, 51.5, 1), name="x")

    # Reindexer le dataframe pour inclure toutes les valeurs de x
    df = df.set_index("x").reindex(full_x)

    # Remplir les y manquants par 0
    df["y"] = df["y"].fillna(0)

    # (Optionnel) Revenir à un dataframe standard
    df = df.reset_index()
