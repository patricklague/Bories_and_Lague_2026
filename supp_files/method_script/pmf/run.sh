...
do
  ...
  mkdir -p popc/distributions6/${aa,,}
  mkdir -p popc/pmfs6/
  mkdir -p pmf-data6/
  for t in "${traj[@]}"
  do
  ...
  python pmf-from-distribution6.py
  mv trajectory*dat popc/distributions6/${aa,,}/
  mv pmf_moyen.dat popc/pmfs6/${aa,,}.dat
  mv pmf.dat pmf-data6/pmf_${aa,,}.dat
done
