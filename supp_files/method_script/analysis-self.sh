...
SCRIPTDIR="/home/bories/Documents/Travail/results/homoPOPC-aa/analyses/scripts2"
...
blocs="0"	# get trajectory separated in blocs
watdel="1"	# delete water for the dcd file
...
aadensProf="1"	# amino acid density profiles
...
if [ "$thickness" = "1" ]; then
  ...
  vmd -dispdev none -e $SCRIPTDIR/thickness.vmd
  ...
  rm bilayer-thickness.dat temp.dat
  echo "    done!"
...
if [ "$aadensProf" != "0" ]; then
  ...
  do
    ...
    rm trajectory.dcd
    #./average_profiles.sh dens_mono_frame_*.dat > dens_mono_avg.dat
    python ./average_profiles.py dens_mono_frame_*.dat > dens_mono_avg.dat
    #./average_profiles.sh dens_nonmono_frame_*.dat > dens_nonmono_avg.dat
    python ./average_profiles.py dens_nonmono_frame_*.dat > dens_nonmono_avg.dat
    ./combine_profiles.sh dens_mono_avg.dat dens_nonmono_avg.dat > dens_all_avg.dat
    mv -f dens_all_avg.dat  $DATA/densityProfiles/profile-aa-$i-mono.dat
    #mv -f profile-aa.dat  $DATA/densityProfiles/profile-aa-$i.dat
    
  done

  rm solutes.vmd dens_mono_frame_*.dat dens_nonmono_frame_*.dat
