...
puts $outfile "#frame monomers_% popc_isolated_%"

# Sélections
set sel_SC_all [atomselect top "resname AAA"]
...
for {set i 0} {$i < $nframes} {incr i} {
    ...
    # ----- SC_ monomères (cutoff 4 Å)
    set monomer_count 0
    foreach resid_SC $SC_resids {
        # voisins SC_ à 4 Å du résidu courant
        set sel_neighbors [atomselect top "resname AAA and not resid $resid_SC and within 4 of (resname AAA and resid $resid_SC)"]
    ...
    puts $outfile "$i $percent_SC $percent_POPC"

    puts "Frame $i: AAA monomers=${percent_SC}% | POPC isolated=${percent_POPC}%"
}
